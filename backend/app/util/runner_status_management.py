...
async def track_runner_state(runner_id: int, request_id: str):
    ...
    while datetime.now() < timeout:
        ...
        with Session(engine) as session:
            ...
            if runner.state in ["ready", "ready_claimed", "active", "terminated", "error"]:
                # Emit final state
                ...
                if runner.state in ["ready", "ready_claimed", "active"]:
                    await runner_status_emitter.emit_status(
                        request_id,
                        "RUNNER_READY",
                        "Runner is ready to use",
                        {"runner_id": runner_id, "url": runner.url}
                    )
...
async def process_runner_event(request_id: str, runner_id: int, event: RunnerHistory):
    ...
    event_map = {
        "runner_created": ("RUNNER_CREATED", "Runner record created"),
        "instance_running": ("INSTANCE_RUNNING", "Virtual machine is now running"),
        "instance_ip_assigned": ("IP_ASSIGNED", "IP address assigned to runner"),
        "ssh_available": ("SSH_AVAILABLE", "SSH connection available"),
        "startup_script_completed": ("STARTUP_COMPLETE", "Initialization script completed")
    }
