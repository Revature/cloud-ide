...
def get_images_by_cloud_connector_id(cloud_connector_id: int) -> list[Image]:
    ...
    return image_repository.find_images_by_cloud_connector_id(cloud_connector_id)
...
def update_image(image_id: int, updated_image) -> bool:
    ...
    existing_image = image_repository.find_image_by_id(image_id, include_deleted=False, include_inactive=True)
    ...
    db_image = image_repository.update_image(image_id, image_data)
    # moved the commit into the update_image function
    # session.commit()
...
def update_runner_pool(image_id: int, runner_pool_size: int) -> bool:
    ...
    # Get the existing image first to check if pool size will change
    existing_image = image_repository.find_image_by_id(image_id)
    if not existing_image:
        logger.error(f"Image with id {image_id} not found for updating")
        return False
    ...
    # Get the updated image from repository
    db_image = image_repository.update_image(image_id, existing_image)
    # moved this into update_image function
    # session.commit()
...
async def update_image_status(image_id: int, is_active: bool) -> Optional[Image]:
    ...
    try:
        ...
        updated_image = image_repository.update_image_status(image.id, new_status)

        # Commit changes - I moved this into the update_iomage_status function
        # session.commit()
...
def get_image_config(image_id: int, initiated_by: str = "default") -> dict:
    ...
    results:dict = {}
    # 1) Fetch the Image.
    db_image : Image = image_repository.find_image_by_id(image_id)
    if not db_image:
    ...
    db_machine = machine_repository.find_machine_by_id(db_image.machine_id)
    if not db_machine:
    ...
    db_cloud_connector = cloud_connector_repository.find_cloud_connector_by_id(db_image.cloud_connector_id)
...
async def create_image(image_data: dict, runner_id: int) -> Image:
    ...
    runner_db_image = image_repository.find_image_by_id(runner.image_id)
    ...
    cloud_connector = cloud_connector_repository.find_cloud_connector_by_id(cloud_connector_id)
    ...
    try:
        ...
        db_image = image_repository.create_image(new_image)
...
async def delete_image(image_id: int) -> bool:
    ...
    # Get the image
    db_image = image_repository.find_image_by_id(image_id, include_deleted=False, include_inactive=True)
    if not db_image:
        logger.error(f"Image with id {image_id} not found")
        raise RunnerExecException(f"Image with id {image_id} not found")

    # Get the cloud connector
    cloud_connector = cloud_connector_repository.find_cloud_connector_by_id(db_image.cloud_connector_id)
    if not cloud_connector:
    ...
    runners = runner_repository.find_runners_by_image_id(image_id)
    ...
    try:
        ...
        if str(deregister_response) == "200":
            ...
            result = image_repository.delete_image(image_id)
            if result:
