...
Scenario: B2 - Runner not found returns 404
  Given url runnersUrl + nonExistentRunnerId
  When method GET
  Then status 404
...
Scenario: C2 - Extend session for non-existent runner returns 404
  Given url runnersUrl + nonExistentRunnerId + '/extend_session'
...
Scenario: D2 - Missing port parameter causes validation error
  Given url runnersUrl + '1/devserver'
  When method GET
  Then status 422
...
Scenario: E5 - Updating state for non-existent runner returns 404
  Given url runnersUrl + nonExistentRunnerId + '/state'
...
Scenario: G4 - Stop runner - Runner not found returns 404
  Given url runnersUrl + nonExistentRunnerId + '/stop'
  When method PATCH
  Then status 404
  And match response.detail == "Runner not found"
...
Scenario: H2 - Start runner - Runner not found returns 404
  Given url runnersUrl + nonExistentRunnerId + '/start'
  When method PATCH
  Then status 404
  And match response.detail == "Runner not found"
